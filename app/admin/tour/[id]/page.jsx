...
export default function ChangeTourPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Chuyển về trang quản lý tour sau khi sửa thành công")
      } else {
        const errorData = await response.json();
        console.error('Lỗi khi sửa tour:', errorData);
        alert('Đã có lỗi xảy ra khi sửa tour.');
      }
    } catch (error) {
      console.error('Lỗi mạng:', error);
      alert('Lỗi mạng, không thể sửa tour.');
    }
  };
 useEffect(() => {
  const fetchTour = async () => {
    ...
    try {
      const response = await fetch(`/api/tour/${id}`);
      if (!response.ok) throw new Error('Không thể tải dữ liệu tour');
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-2xl font-bold mb-4">Sửa thông tin Tour</h1>
